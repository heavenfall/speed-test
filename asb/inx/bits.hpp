#ifndef GMLIB_BITS_HPP_INCLUDED
#define GMLIB_BITS_HPP_INCLUDED
...
///
template <typename Type>
constexpr Type make_mask_limit(size_t Count, size_t Offset = 0) noexcept
{
	static_assert(std::is_integral<Type>(), "Type must be integral");
...
template <typename Type, size_t Count, size_t Offset = 0>
constexpr Type make_mask_limit() noexcept
{
	static_assert(Count + Offset <= sizeof(Type) * byte_size, "Mask exceeds Type bit count");
...
///
template <typename Type>
constexpr Type make_msb_mask(size_t Count, size_t Offset = 0) noexcept
{
	static_assert(std::is_integral<Type>(), "Type must be integral");
	if (Count == sizeof(Type) * byte_size)
		return static_cast<Type>(~static_cast<std::make_unsigned_t<Type>>(0));
...
template <typename Type, size_t Count, size_t Offset = 0>
constexpr Type make_msb_mask() noexcept
{
	static_assert(Count + Offset <= sizeof(Type) * byte_size, "Mask exceeds Type bit count");
...
///   size_t Offset: mask offset from lsb
template <typename Type>
constexpr Type make_msb_mask_limit(size_t Count, size_t Offset = 0) noexcept
{
	static_assert(std::is_integral<Type>(), "Type must be integral");
	assert(Count < sizeof(Type) * byte_size);
	return static_cast<Type>(~(~static_cast<std::make_unsigned_t<Type>>(0) >> Count) >> Offset);
...
template <typename Type, size_t Count, size_t Offset = 0>
constexpr Type make_msb_mask_limit() noexcept
{
	static_assert(Count + Offset <= sizeof(Type) * byte_size, "Mask exceeds Type bit count");
...
///
/// bit_shift_from_to: shift bit variable to from variable from, all bits before from are cleared
///   From: bit shift from
///   To: bit shift to
///
template <typename Type>
constexpr Type bit_shift_from_to(Type Value, size_t From, size_t To) noexcept
{
	assert(From < sizeof(Type) * byte_size);
	assert(To < sizeof(Type) * byte_size);
...
template <typename Type>
constexpr Type bit_nshift_from_to(Type Value, size_t From, size_t To) noexcept
{
	assert(From < sizeof(Type) * byte_size);
	assert(To < sizeof(Type) * byte_size);
